...
//! authentication, RPC wiring and group fanout (iterate + call
//! `append_message_in_tx` once per member in its own transaction).
...
/// Refuses before any mutation if the payload would make this message
/// unframeable rather than silently truncating it.
fn enforce_message_size(body: Option<&str>, payload: Option<&Value>) -> Result<(), RpcError> {
    let mut size = body.map_or(0, str::len);
    if let Some(payload) = payload {
        size += serde_json::to_vec(payload)
            .map_err(|_| error::invalid_argument("Message payload is not serializable."))?
            .len();
    }
    if size > RESPONSE_BUDGET_BYTES {
        return Err(error::invalid_argument(
            "Message body/payload exceeds the response budget.",
        ));
    }
    Ok(())
}
...
pub(crate) fn append_message_in_tx(
    ...
    enforce_message_size(new.body, new.payload)?;
...
fn decode_row(row: &rusqlite::Row) -> rusqlite::Result<StoredMessage> {
    ...
    let from = match from_kind.as_str() {
        "coordinator" => Actor::Coordinator(from_coordinator_id.unwrap_or_default()),
        _ => Actor::Dispatch(from_dispatch_id.unwrap_or_default()),
    };
    ...
    let kind = message_kind_from_str(&kind).map_err(|_| {
        rusqlite::Error::FromSqlConversionFailure(
            2,
            rusqlite::types::Type::Text,
            "invalid stored kind".into(),
        )
    })?;
    Ok(StoredMessage {
...
pub(crate) fn message_columns() -> &'static str {
    "message_id, sequence, kind, from_kind, from_coordinator_id, from_dispatch_id,
     to_dispatch_id, subject, body, payload_json, thread_id"
}
