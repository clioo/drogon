/* MIT Copyright (c) 2026 Lovecast Inc. Ported from Orca
   src/renderer/src/components/sidebar/worktree-manual-order.ts at pinned
   source c9790628 (clioo/drogon-orca). Only this header and the module name
   were adapted, plus one deferred export: expandDraggedWorktreeIdsForVisibleLineage
   belongs to the pointer-drag lineage family that C02-B ports with its consumer. */
import {
  buildSparseManualOrderUpdates,
  type WorktreeManualOrderUpdate,
} from "./manual-order-ranks";

export type { WorktreeManualOrderUpdate } from "./manual-order-ranks";
...
export function buildManualOrderUpdatesForVisibleGroups(args: {
  ...
  for (const group of args.groups) {
    ...
    if (
      group.key === args.sourceGroupKey &&
      !arraysEqual(ids, group.worktreeIds)
    ) {
      changed = true;
    }
...
export function buildManualOrderUpdatesForGroupDrop(args: {
  ...
  for (const group of args.groups) {
    ...
    if (group.key === args.targetGroupKey) {
      const boundedDropIndex = Math.max(
        0,
        Math.min(group.worktreeIds.length, args.dropIndex),
      );
      let removedBeforeDrop = 0;
...
export function shouldWriteManualOrderForGroupDrop(args: {
  ...
  return (
    ...
    args.sourceGroupKeys.every(
      (sourceGroupKey) => sourceGroupKey === args.targetGroupKey,
    )
  );
}
