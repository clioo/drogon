...
// src/renderer/src/components/right-sidebar/source-control/commit/discard-confirmation.ts.
// Adapter: local basename helper; literal copy strings (no i18n).
import { basename } from "./panel-class-names";
...
export function getDiscardEntryConfirmationCopy(
  entry: Pick<SourceControlEntry, "area" | "path" | "status">,
): DiscardConfirmationCopy {
  const name = basename(entry.path);

  // Why: untracked and newly-added paths have no HEAD version to restore.
  // The discard path removes the working-tree file in those cases.
