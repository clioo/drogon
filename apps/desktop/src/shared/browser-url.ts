...
// Adaptations (repo contracts; behavior unchanged):
// - ORCA_BROWSER_BLANK_URL ('data:text/html,') is DROGON_BROWSER_BLANK_URL
//   ('about:blank'): this build's guest starts on about:blank (main
//   browser-host HOME_URL) and every blank check uses it.
// - SEARCH_ENGINE_URLS is exported (the source keeps it private); the
//   address-bar task names it as ported API.
// - query.toWellFormed() (ES2024) goes through toWellFormedQuery: this
//   repo's tsconfig lib is ES2023 (coordinator-owned), so the method is
//   untyped here. Same semantics — lone surrogates become U+FFFD before
//   encodeURIComponent (which would throw on them).

/** Blank-tab identity: the guest starts here and blank tabs show no URL. */
export const DROGON_BROWSER_BLANK_URL = "about:blank";
...
export function normalizeKagiSessionLink(rawLink: string): string | null {
  ...
  try {
    ...
    // Why: collapse any duplicate token params so we don't echo two Bearer [REDACTED]
...
export function redactKagiSessionToken(rawUrl: string): string {
  try {
    ...
    if (
      ...
      // Why: Kagi private-session links carry an account Bearer [REDACTED] Strip it
...
function toWellFormedQuery(query: string): string {
  const candidate = query as unknown as { toWellFormed?: () => string };
...
export function buildSearchUrl(
  ...
  return `${SEARCH_ENGINE_URLS[engine]}${encodeURIComponent(toWellFormedQuery(query))}`;
}
...
function absolutePathToFileUrl(filePath: string): string {
  ...
  return normalizedPath.startsWith("/") ? `file://${segments.join("/")}` : `file:///${segments.join("/")}`;
}
...
export function normalizeBrowserNavigationUrl(
  ...
  if (trimmed.length === 0 || trimmed === "about:blank" || trimmed === DROGON_BROWSER_BLANK_URL) {
    return DROGON_BROWSER_BLANK_URL;
  }
...
export function normalizeExternalBrowserUrl(rawUrl: string): string | null {
  ...
  if (normalized === null || normalized === DROGON_BROWSER_BLANK_URL) {
    return null;
  }
