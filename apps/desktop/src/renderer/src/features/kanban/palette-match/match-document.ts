/* MIT Copyright (c) 2026 Lovecast Inc. Ported verbatim from Orca
   src/renderer/src/lib/palette-match/match-document.ts at pinned source c9790628
...
import {
  createPaletteQueryToken,
  type PaletteQueryToken,
} from "./palette-query";
import {
...
function better(a: TokenCandidate | null, b: TokenCandidate): TokenCandidate {
  ...
  return paletteMatchQualityRank(a.quality) <=
    paletteMatchQualityRank(b.quality)
    ? a
    : b;
}
...
function toCandidate(hits: readonly FieldHit[]): TokenCandidate {
  ...
  for (const hit of hits) {
    if (
      paletteMatchQualityRank(hit.match.quality) >
      paletteMatchQualityRank(quality)
    ) {
...
function matchCompositePairs(
  ...
  for (const pair of document.compositePairs) {
    const leftField = document.fields.find(
      (field) => field.id === pair.leftFieldId,
    );
    const rightField = document.fields.find(
      (field) => field.id === pair.rightFieldId,
    );
  ...
  for (const field of document.fields) {
    ...
    if (!field.evidenceId) {
      ...
      candidates.byEvidenceId.set(
        field.evidenceId,
        better(
          candidates.byEvidenceId.get(field.evidenceId) ?? null,
          candidate,
        ),
      );
    }
...
function buildAssignments(
  ...
  for (let index = 0; index < candidates.length; index += 1) {
    ...
    const evidence = evidenceId
      ? (candidate.byEvidenceId.get(evidenceId) ?? null)
      : null;
    const chosen = evidence
      ? better(candidate.visible, evidence)
      : candidate.visible;
    if (!chosen) {
      return null;
    }
...
function rankAssignments(args: {
  ...
  for (const assignment of args.assignments) {
    if (
      paletteMatchQualityRank(assignment.quality) >
      paletteMatchQualityRank(worstQuality)
    ) {
  ...
  const isContainerOnly =
    ...
    containerOnlyTokenCount === matchedTokenCount;
...
export function matchPaletteDocument(args: {
  ...
  const evidenceIds: (string | null)[] = [
    null,
    ...document.evidenceUnits.keys(),
  ];
