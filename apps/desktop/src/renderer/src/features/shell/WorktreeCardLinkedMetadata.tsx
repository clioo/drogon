 ...
 * Owner's guide: creator provenance renders as a compact secondary pill, not
 * a dominating line — the fact stays visible (and toggleable) without
 * repeating full-strength metadata on every card. */
export function WorktreeCardLinkedMetadata({ worktree, properties, ports, issueLinks = [], onOpenIssue }: {
  ...
  const provenance: { text: string; title: string }[] = [];
  const labels: { text: string; title: string }[] = [];
  if (worktree.creator === "cli" && properties.cli !== false)
    provenance.push({ text: "Drogon CLI", title: "Created through Drogon CLI" });
  if (worktree.creator === "automation" && properties.automation !== false)
    provenance.push({ text: "Automation", title: "Created by an automation" });
  ...
  if (provenance.length === 0 && labels.length === 0 && issues.length === 0) return null;
  return <span className="shell-worktree-card-meta shell-worktree-card-linked-meta">
    {provenance.map(({ text, title }) => <span className="shell-worktree-card-provenance" key={title} title={title}>{text}</span>)}
