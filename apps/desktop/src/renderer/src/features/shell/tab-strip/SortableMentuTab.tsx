/* MIT Copyright (c) 2026 Lovecast Inc. Ported from Orca's
   src/renderer/src/components/tab-bar/RecipeTab.tsx (the sortable root and
   its menu discipline) via this repo's SortableBrowserTab/SortableEditorTab
   split. Adapter: the row chrome stays in MentuStripTab (this wrapper only
   owns useSortable and the shared TabContextMenu); the fork's recipe tab
   offers the same Pin/Close/Close-variants menu as any other tab, with no
   recipe-specific rows. */
...
import { MentuStripTab } from "./MentuStripTab";

/**
 * The Mentu tab as a draggable tab-strip tab. Activation still runs on
 * click; a press that travels past the drag threshold is a drop and the
 * click is suppressed at capture so a reorder never switches surface.
 */
export function SortableMentuTab({
  isActive,
  isPinned,
  hasTabsToRight,
  hasTabsToLeft,
  ...
  const { setNodeRef, listeners } = useSortable({ id: "mentu-tab" });
