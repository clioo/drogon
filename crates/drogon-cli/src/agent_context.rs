    ...
    let mut commands = vec![
        ...
        entry(
            ...
            &[
                ...
                "The Return is delivered as its own keypress after the body, and for a far end that reads a paste as text (bracketed paste on, normal screen) the body is wrapped in paste markers first, so a paste-detecting TUI cannot swallow the Return mid-turn.",
                "A full-screen application on the alternate screen (vim, less, htop) is never framed: a paste there is text in a buffer, not the keystrokes you meant. The result's bracketedPaste says which happened.",
