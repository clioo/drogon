/* MIT Copyright (c) 2026 Lovecast Inc. Ported verbatim from the read-only
   reference's src/shared/workspace-status-defaults.ts. Root-approved
   source semantics (see the shared Workspace Options module/RPC contract
   proposal): the exact default status set, ids, labels, colors and icons,
   unchanged. */
import type { WorkspaceStatusDefinition } from "./persistence-contracts/worktree-types";
