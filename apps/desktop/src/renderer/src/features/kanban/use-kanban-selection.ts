/* MIT Copyright (c) 2026 Lovecast Inc. Ported from Orca
   src/renderer/src/components/sidebar/use-workspace-kanban-selection.ts at
   pinned source c9790628 (clioo/drogon-orca). Only this header and the
   KanbanWorktree / host-identity / multi-selection import paths were
   adapted. */
...
export function useWorkspaceKanbanSelection(
  open: boolean,
  boardWorktrees: readonly KanbanWorktree[],
  renderedWorktrees: readonly KanbanWorktree[] = boardWorktrees,
) {
  ...
  const [selectedWorktreeIds, setSelectedWorktreeIds] = useState<Set<string>>(
    new Set(),
  );
