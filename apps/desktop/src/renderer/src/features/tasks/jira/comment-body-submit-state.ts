...
// src/renderer/src/lib/comment-body-submit-state.ts — verbatim behavior;
// the UTF-8 helpers from the fork's shared/utf8-byte-limits are inlined
// (TextEncoder is the whole of what they model).
...
function getCommentBodyPresence(
  ...
  for (let index = 0; index < body.length; index += 1) {
    ...
    scannedBytes += encoder.encode(String.fromCodePoint(codePoint)).byteLength;
