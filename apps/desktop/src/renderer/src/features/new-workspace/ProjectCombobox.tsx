/* MIT Copyright (c) 2026 Lovecast Inc. Ported from Orca's
   ...
   (adapter: options come from ./project-combobox-options over the daemon
   project list, recency from ./use-recent-project-ids over this repo's
   ProjectGroup worktrees, and the source's i18n keys are inlined as their
   English literals — Drogon has no i18n layer). */
...
type ProjectComboboxProps = {
  options: readonly ComposerProjectOption[];
  /** Project groups, only for the source's recency ordering. */
...
export default function ProjectCombobox({
  ...
  const recentIds = useRecentProjectIds(groups);
  ...
  const deriveRowKeys = useCallback(
    (query: string): string[] => [
      ...sectionProjectOptions(rankProjectOptions(options, query, recentIds), query, recentIds)
