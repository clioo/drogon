/* MIT Copyright (c) 2026 Lovecast Inc. Ported verbatim from the read-only
   reference's src/shared/workspace-status-default-migration.ts.
   Root-approved source semantics: kept even though Drogon (a from-zero
   rewrite) has never shipped the specific short-lived intermediate builds
   this detects -- the four `_workspaceStatuses*` migration stamps this
   backs are part of the shared persisted-UI-state contract
   (persistence-contracts/persisted-ui-state-types.ts), so the exact-payload
   detection stays available for any future Drogon build that repeats one
   of these historical shapes, rather than inventing a different migration
   mechanism. */
...
const CONDUCTOR_DEFAULT_STATUS_VISUALS: Record<
  string,
...
const LEGACY_DEFAULT_STATUS_VISUALS: Record<
  string,
  { color: string; icon: string }
> = {
  todo: { color: "neutral", icon: "circle" },
...
const WORKFLOW_DEFAULT_STATUS_IDS = [
  "completed",
  "in-review",
  "in-progress",
  "todo",
