...
const NAMES = ['react', 'react-dom', 'happy-dom'];
const digest = (file) => createHash('sha256').update(readFileSync(file)).digest('hex');

export function resolveRendererRuntime(declaration, sourceRoot) {
  if (declaration === undefined) return null;
  if (!declaration || Object.keys(declaration).sort().join() !== [...NAMES].sort().join()) {
    throw new Error('Renderer runtime must pin react, react-dom and happy-dom versions only.');
  }
  ...
  return NAMES.map((name) => {
    ...
    if (typeof declaration[name] !== 'string' || metadata.name !== name || metadata.version !== declaration[name]) {
      throw new Error(`Renderer runtime version mismatch for ${name}.`);
    }
    return { name, version: metadata.version, packageFile, packageSha256: digest(packageFile) };
  });
}

export function stageRendererRuntime(runtime, capsuleRoot) {
  if (!runtime) return;
...
export function verifyRendererRuntime(runtime, capsuleRoot) {
  if (!runtime) return;
  for (const entry of runtime) {
    ...
    if (realpathSync(linked) !== entry.packageFile || digest(linked) !== entry.packageSha256) {
      throw new Error(`Renderer runtime changed since staging: ${entry.name}.`);
    }
  }
}
