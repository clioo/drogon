/* MIT Copyright (c) 2026 Lovecast Inc. Ported verbatim from Orca
   src/renderer/src/lib/palette-match/text-segments.ts at pinned source c9790628
...
function isCamelBoundary(text: NormalizedText, index: number): boolean {
  ...
  return (
    UPPERCASE.test(original[here] ?? "") &&
    !UPPERCASE.test(original[before] ?? "")
  );
}

function splitComponentIntoWords(
  text: NormalizedText,
  ...
  words.push({
    ...
    text: normalized.slice(start, component.end),
  });
  return words;
}
...
function buildAtom(
  ...
  for (let index = start; index <= end; index += 1) {
    ...
    if (inRun) {
      ...
      components.push({
        ...
        text: normalized.slice(runStart, index),
      });
      runStart = -1;
    }
  }
  ...
  return {
    ...
    components,
    compact,
    compactOffsets: Int32Array.from(compactOffsets),
  };
}
