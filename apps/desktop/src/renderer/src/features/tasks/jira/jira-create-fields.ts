 ...
 */
export function getJiraUserCreateFieldKeys(
...
export function findJiraCreateAllowedValue(field: JiraCreateField, draftValue: string) {
  return field.allowedValues?.find((value) => {
    return (
      value.id === draftValue || value.value === draftValue || value.name === draftValue
    );
  });
}
...
export function buildJiraCreateFieldValue(
  ...
  if (field.allowedValues?.length) {
    return getJiraCreateOptionPayload(
      findJiraCreateAllowedValue(field, trimmed),
      trimmed,
    );
  }
