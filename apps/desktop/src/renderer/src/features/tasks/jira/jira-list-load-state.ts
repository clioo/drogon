...
// src/renderer/src/components/task-page-jira-load-state.ts — the error
// message parsing, status-code heuristics and summary copy are the fork's.
// The Tasks page feeds it the daemon's Result error message (see
// use-jira-list-state.ts), so a thrown-error boundary is unnecessary.
...
export function createTaskPageJiraLoadFailureState(
  error: unknown,
): TaskPageJiraLoadFailureState {
