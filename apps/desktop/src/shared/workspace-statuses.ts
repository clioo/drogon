/* MIT Copyright (c) 2026 Lovecast Inc. Ported from the read-only
   reference's src/shared/workspace-statuses.ts. Root-approved source
   semantics (see the shared Workspace Options module/RPC contract
   proposal): same normalization/clamp/lookup behavior, unchanged. Adapted
   only for this repo's file layout -- `WorkspaceStatus`/
   `WorkspaceStatusDefinition` come from persistence-contracts/worktree-types
   (already ported there) and `Worktree` from session-contract, in place of
   the reference's `./worktree/types`. */
import type {
  WorkspaceStatus,
  WorkspaceStatusDefinition,
} from "./persistence-contracts/worktree-types";
import type { Worktree } from "./session-contract";
import {
  DEFAULT_STATUS_VISUALS,
  DEFAULT_WORKSPACE_STATUSES,
} from "./workspace-status-defaults";
import {
