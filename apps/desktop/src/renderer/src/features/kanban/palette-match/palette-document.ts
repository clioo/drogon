/* MIT Copyright (c) 2026 Lovecast Inc. Ported verbatim from Orca
   src/renderer/src/lib/palette-match/palette-document.ts at pinned source c9790628
 ...
 */
export type PaletteCompositePair = {
  leftFieldId: string;
  rightFieldId: string;
};
...
export function buildPaletteDocument(
  input: PaletteDocumentInput,
): PaletteDocument {
  ...
  for (const entry of input.evidence) {
    const fields = entry.fields.filter(
      (field): field is PaletteEvidenceFieldSource =>
  ...
  return {
    ...
    compositePairs: (input.compositePairs ?? []).filter(
      (pair) =>
        fieldIds.has(pair.leftFieldId) && fieldIds.has(pair.rightFieldId),
    ),
    evidenceUnits,
...
export function comparePaletteDocumentRank(
