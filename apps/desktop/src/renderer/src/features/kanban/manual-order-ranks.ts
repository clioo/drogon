/* MIT Copyright (c) 2026 Lovecast Inc. Ported verbatim from Orca
   src/renderer/src/components/sidebar/worktree-manual-order-ranks.ts at
   pinned source c9790628 (clioo/drogon-orca). Only this header was added. */
...
function buildFallbackManualOrderUpdates(
  ...
  for (let index = 0; index < orderedIds.length; index++) {
    updates.set(orderedIds[index]!, {
      manualOrder: now - index * MANUAL_ORDER_STRIDE,
    });
  }
  return updates;
}
...
export function buildSparseManualOrderUpdates(args: {
  ...
  if (
    args.allWorktreeIds.some(
      (id) => getManualOrderRank(args.rankByWorktreeId!, id) === null,
    )
  ) {
    return buildFallbackManualOrderUpdates(
      mergeVisibleOrderIntoKnownOrder(args.allWorktreeIds, args.orderedIds),
  ...
  const lastMovedIndex = args.orderedIds.findLastIndex((id) =>
    movedSet.has(id),
  );
  const beforeId = args.orderedIds
    .slice(0, firstMovedIndex)
    .findLast((id) => !movedSet.has(id));
  const afterId = args.orderedIds
    .slice(lastMovedIndex + 1)
    .find((id) => !movedSet.has(id));
  ...
  if (beforeRank === null && afterRank === null) {
    ...
    const start = Math.max(
      args.now,
      afterRank! + orderedMovedIds.length * MANUAL_ORDER_STRIDE,
    );
    for (let index = 0; index < orderedMovedIds.length; index++) {
