...
### Devicons mixed attribution reconciled at source scope

`e5-devicons-legacy-origin.json` records the whole18-SVG legacy subtree. Seventeen Nerd files are byte-identical to the matching `!SVG/` files at Vorillaz revision`ba75593fdf8d66496676a90cbf127d721f73e961`. Dropbox differs; Nerd's [introducing commit](https://github.com/ryanoasis/nerd-fonts/commit/1b34bab199aace815e9d23ddda01fd84bb6ff201) explicitly documents its hand modification and its blob matches the pinned Nerd file. This is17 exact copies plus one documented derivative, not18 exact matches.

The [historical Vorillaz README](https://github.com/vorillaz/devicons/blob/ba75593fdf8d66496676a90cbf127d721f73e961/README.md) identifies Theodore Vorillas and declares the icon font MIT, with a separate brand disclaimer. The inspected Nerd generator combines modern Deviconv2.16.0, legacy Vorillaz files and fixed import-problem SVGs. Its fixed README documents those modifications. Preserve the modern konpa MIT notice, legacy attribution/declaration and Nerd modification attribution together. The modern license alone does not describe the complete set. These bounded notices/origin statements are now identified; exact reconstruction of all glyph transformations and final packaging remain unproved.

All18 source/target payloads were compared with exact SHA256 and byte equality, in three bounded concurrent fetches. The catalog records seven generator/mapping/README fingerprints and the precise modification pointer. No downloaded generator was executed, no asset was changed, and no product test or rendered-geometry equivalence is claimed. A first history query used `SVG`, while the actual historical directory is `!SVG`; a bounded dated snapshot resolved it without an absence-of-origin claim.

**Seti remains distinct.** The inspected v0.8.1 candidate contains one aggregate SVG font rather than Nerd's191 standalone SVGs; none is byte-identical as a file. Its Jesse Weed MIT notice is identified, but this does not bind every current Seti/Original glyph to that particular release. The source generator combines distinct Seti and custom Original inputs and applies scaling/centering. Preserve that lineage/notice question instead of treating the aggregate version, family name or lack of direct file matches as proof of either identity or infringement.
