...
//   src/renderer/src/lib/pane-manager/pane-terminal-tui-wheel-reports.ts
// Exact wheel-distance normalization and report-count policy for fullscreen TUIs.
...
export function resolveTerminalWheelDirection(
  event: Pick<WheelEvent, "deltaY">,
): -1 | 1 {
  return event.deltaY < 0 ? -1 : 1;
}

function legacyVerticalWheelDelta(
  event: TerminalTuiWheelEventInput,
): number | null {
  const wheelEvent = event as WheelEventWithLegacyDelta;
  if (
    typeof wheelEvent.wheelDeltaY === "number" &&
    Number.isFinite(wheelEvent.wheelDeltaY)
  ) {
    return wheelEvent.wheelDeltaY;
  }
  if (
    typeof wheelEvent.wheelDelta === "number" &&
    Number.isFinite(wheelEvent.wheelDelta)
  ) {
...
function hasDiscreteLegacyWheelDelta(
  event: TerminalTuiWheelEventInput,
): boolean {
  const legacyDelta = legacyVerticalWheelDelta(event);
  return (
    legacyDelta !== null &&
    Math.abs(legacyDelta) >= LEGACY_MOUSE_WHEEL_DELTA_MIN
  );
}

export function isDiscreteTerminalTuiWheelEvent(
  event: TerminalTuiWheelEventInput,
): boolean {
...
function isTrackpadLikePixelWheelEvent(
  event: TerminalTuiWheelEventInput,
): boolean {
  return (
...
function resolveWheelDistanceRows(
  ...
  const rowsFromLegacy =
    legacyDelta === null
      ? 0
      : Math.abs(legacyDelta) / LEGACY_MOUSE_WHEEL_DELTA_UNIT;
...
function resolveBurstWheelDistanceRows(
  ...
  state.fastStreak = Math.min(
    TUI_WHEEL_BURST_RAMP_EVENTS,
    state.fastStreak + 1,
  );

  return (
    TUI_WHEEL_BURST_MAX_BONUS_ROWS *
    cadence *
    (state.fastStreak / TUI_WHEEL_BURST_RAMP_EVENTS)
  );
}
...
export function normalizeTerminalTuiMouseWheelMultiplier(
...
export function resolveTerminalTuiMouseWheelReportCount(
  ...
  const trackpadReportCount = resolveTrackpadPixelWheelReportCount(
    event,
    state,
    distanceRows,
  );
