/* MIT Copyright (c) 2026 Lovecast Inc. Ported from Orca
   ...
   at pinned source c9790628 (clioo/drogon-orca). Only this header and the
   drag-data import path were adapted. */
import { useEffect } from "react";
import type React from "react";
import { hasWorkspaceDragData } from "./drag-data";

function getWheelPixels(event: WheelEvent): number {
  const unit =
    event.deltaMode === WheelEvent.DOM_DELTA_LINE ? 16 : window.innerHeight;
...
export function useWorkspaceKanbanShiftWheelScroll(
  ...
  useEffect(() => {
    ...
    const handleDragStart = (event: DragEvent): void => {
      isWorkspaceDragActive = event.dataTransfer
        ? hasWorkspaceDragData(event.dataTransfer)
        : false;
      lastDragPoint = isWorkspaceDragActive
    ...
    const handleWheel = (event: WheelEvent): void => {
      ...
      if (
        ...
        (!isEventInsideElement(event, board) &&
          !pointIsInsideElement(lastDragPoint, board))
      ) {
        return;
      }
