/* MIT Copyright (c) 2026 Lovecast Inc. Ported verbatim from Orca
   src/renderer/src/lib/palette-match/indexed-field.ts at pinned source c9790628
...
export type PaletteFieldProfile =
  "structured-label" | "identifier" | "path" | "prose" | "exact-alias";
 ...
 */
export type PaletteIdentifierKind =
  "number" | "port" | "sha" | "version" | "date" | "key";
...
const IDENTIFIER_PREFIX_KINDS: ReadonlySet<PaletteIdentifierKind> =
  new Set<PaletteIdentifierKind>(["port", "sha", "key"]);

export function identifierKindAllowsPrefix(
  kind: PaletteIdentifierKind,
): boolean {
...
const QUALITIES_BY_PROFILE: Record<
  PaletteFieldProfile,
