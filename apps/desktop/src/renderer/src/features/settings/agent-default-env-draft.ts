...
// Ported from src/renderer/src/components/settings/agent-default-env-draft.ts.
const isClipboardTextByteLengthOverLimit = (text: string, max: number) =>
...
export function stringifyAgentDefaultEnvDraft(
  env: Record<string, string>,
): string {
...
export function parseAgentDefaultEnvDraft(
  value: string,
): AgentDefaultEnvDraftParseResult {
  if (
    isClipboardTextByteLengthOverLimit(value, AGENT_DEFAULT_ENV_DRAFT_MAX_BYTES)
  ) {
