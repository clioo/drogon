...
import {
  classifyMentuRunStatus,
  LOCAL_RUN_EVIDENCE,
  parseMentuRecipeJson,
  serializeMentuRecipeDocument,
  stringifyMentuRecipeDocument
} from './mentu-recipe-contract'

describe('Mentu recipe contract', () => {
  ...
  it('keeps local evidence distinct from formal Commitment Protocol records', () => {
    expect(LOCAL_RUN_EVIDENCE).toEqual({
      kind: 'local-run-evidence',
      label: 'Local run evidence',
      formalCommitmentProtocol: false
    })
    expect(classifyMentuRunStatus('running')).toBe('running')
    expect(classifyMentuRunStatus('warn_bookkeeping')).toBe('warning')
    expect(classifyMentuRunStatus('ok')).toBe('successful')
    expect(classifyMentuRunStatus('failed')).toBe('failed')
    expect(classifyMentuRunStatus('future-outcome')).toBe('invalid')
  })
})
