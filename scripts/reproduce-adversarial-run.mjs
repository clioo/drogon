...
async function processRows() {
  const { stdout } = await exec("/bin/ps", ["-axo", "pid=,ppid=,lstart=,command="], { timeout: 20_000 });
  return stdout
    .trim()
    .split("\n")
    .map((line) => {
  ...
  for (const [pid, identity] of owned) {
    ...
    if (row && row.identity === identity) {
      ...
      verdicts.push({
        pid,
        identity,
        verdict: row && row.identity === identity ? "live" : "exited",
        note: "terminated by the run that started it",
      });
      continue;
    }
