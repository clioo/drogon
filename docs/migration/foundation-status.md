...
- Sonnet 5: desktop harness task `task_c9a9e4fe678b`, dispatch `ctx_c925b537603b`, settled and was released. Coordinator subsequently fixed rendered popover opening/alignment, unknown catalog compatibility, prompt preservation and harness labels. No active desktop writer remains.
- GLM-5.3-Flash: native CLI harness task `task_94273ee24abd`, dispatch `ctx_ff631d99be5a`, settled and was released. Its 109 tests, formatting and strict Clippy passed independently. No active CLI writer remains.
...
Further filesystem review added database/sidecar symlink and hard-link refusal, SQLite no-follow opening after resolving legitimate platform directory aliases, and rejection of lossy workspace paths. Core/service now pass 43 macOS tests; the real non-UTF-8 filesystem target case is Linux-only because APFS refuses creating that name, with a separate pure conversion test on Unix. The idle-capacity test now verifies a real close instead of treating a read timeout as EOF and tolerates the legitimate macOS closed-peer socket-option race. Strict Clippy and the 19-check acceptance passed: `.preflight/acceptance/core-cli-1788641272620-5df4e12f-2413-4332-b92e-137b9a4d000d.json`.

## Deliberate first-slice limits

The completed harness slice passed the full Rust workspace (166 macOS tests), strict Clippy, formatting and debug build. Electron passed typecheck, 44 unit tests and production build. At 20:59 UTC, real rendered acceptance passed all nine checks, including native Pi launch, exact reload/stop, trigger alignment, light/dark/narrow form captures and Escape dismissal: `.preflight/acceptance/desktop-1788641953155-a7b35e9a-0c4a-4173-b624-5269d043e1f0/report.json`. The corrected screenshots were independently inspected. The typed CLI passed the expanded 19-check suite: `.preflight/acceptance/core-cli-1788641963408-07b909d3-33bf-4a27-aba1-695e8bb577e9.json`; the same harness admission was replayed through both typed CLI and raw diagnostic RPC without a second session. Cleanup confirmed the owned sessions, Electron and service exited. These Pi probes sent no inference prompt; Pi did download its `fd` startup dependency into the isolated fixture, so they are not no-network tests. The [latest committed-core CI](https://github.com/clioo/drogon/actions/runs/33991262242) also passed; it predates the desktop/typed-harness checkpoint.

Explicit service startup; no bundled installer yet. Only local native IPC is implemented so far; Windows service, SSH and mixed-host execution still require implementation and actual evidence. Native harness discovery/launch works through the typed CLI and UI; installed discovery does not prove authentication, model availability or DGX connectivity. No Git operations beyond folder/repository registration, orchestration, skills integration, Mentu, Bots or Meetings are represented as working in the new UI. The migration note makes unavailable features explicit. Bounded session resource retirement, launch-request recovery after the form/window is closed, and additional persistence-failure cases still need review before release.
