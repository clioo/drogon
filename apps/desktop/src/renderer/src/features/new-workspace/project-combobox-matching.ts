/* MIT Copyright (c) 2026 Lovecast Inc. Ported from Orca's
   ...
   (adapter: the option type comes from ./project-combobox-options over the
   daemon project list; the source's NewWorkspaceProjectOption carries the
   same kind/displayName/detail fields). */
import {
  isComposerProjectOptionQueryTooLarge,
  type ComposerProjectOption,
} from "./project-combobox-options";

export type ScoredProjectOption = {
  option: ComposerProjectOption;
  score: number;
...
export function rankProjectOptions(
  ...
  if (isComposerProjectOptionQueryTooLarge(rawQuery)) {
    return [];
  }
  ...
  const sections: ProjectOptionSection[] = [
    ...
    },
    {
      ...
      items: matches.filter(
        (m) => m.option.kind === "project-group" && !recentSet.has(m.option.id),
      ),
    },
  ];
