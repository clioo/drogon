// R17-C: port of the fork's task-page-jira-create-fields tests
...
import type { JiraCreateField } from "../../../../../shared/jira-contract";
...
describe("isVisibleJiraCreateField", () => {
  const cases = [
    ...
    // characterization: current behavior — the system-field filter is
    // exact-match and case-sensitive, so a differently-cased key stays
    // visible.
...
describe("isJiraScalarUserCreateField", () => {
  ...
  it("rejects array-of-user, which the single-value picker would collapse to one member", () => {
    expect(
      isJiraScalarUserCreateField(field({ schema: { type: "array", items: "user" } })),
    ).toBe(false);
  });
...
describe("getJiraCreateOptionPayload", () => {
  ...
  it("skips empty-string members", () => {
    // characterization: current behavior — the checks are truthiness-based,
    // so an empty id falls through to the next candidate.
...
describe("buildJiraCreateFieldValue", () => {
  ...
  it("prefers allowed values over the number branch", () => {
    // characterization: current behavior — the allowedValues branch runs
    // before the number branch, so the draft stays a string even on a
    // number-typed field.
