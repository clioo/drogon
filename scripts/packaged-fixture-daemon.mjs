...
/** Drogon variables intentionally consumed by the acceptance launcher. All
 * other DROGON_* variables are worker/runtime context and must not cross the
 * acceptance boundary. Paths generated by this run (data/profile/fixtures)
 * are added after sanitization and are therefore not inherited allowlist
 * entries.
 */
export const ACCEPTANCE_ENV_ALLOWLIST = new Set([
  "DROGON_BACKGROUND_WINDOW",
  "DROGON_MIXED_FROM_BUNDLE",
  "DROGON_PROBE_SURFACES",
  "DROGON_SKIP_MODEL_JOURNEYS",
  "DROGON_UPGRADE_FROM_BUNDLE",
  "DROGON_VERIFY_OS_FOCUS",
]);
 ...
 * child RPCs to the worker. Scrub the entire namespace and preserve only the
 * explicit acceptance flags listed above.
 */
export function cleanAcceptanceEnvironment(source = process.env) {
  const clean = { ...source };
  for (const key of Object.keys(clean)) {
    if (key.startsWith("DROGON_") && !ACCEPTANCE_ENV_ALLOWLIST.has(key))
      delete clean[key];
  }
  return clean;
}
