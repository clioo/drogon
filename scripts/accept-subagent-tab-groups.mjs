...
/**
 * The two projections the run reads the page through, as pure functions of
 * what the DOM said. Exported so their shape is testable without launching
 * an app: a silent change here would turn every assertion below into a
 * tautology over `undefined`.
 */
export function projectStripTab(raw) {
  return {
    id: raw.id ?? null,
...
/** The session id an `aria-labelledby` on the active panel points at. */
export function promptTabIdFromLabel(labelled) {
 ...
 *
 * The page closure only reads attributes and the projection happens here,
 * in Node: the packaged renderer ships `script-src 'self'` and rejects
 * string evaluation, so a page closure can never call an imported helper
 * (same rule as acceptance-terminal-text.mjs).
 */
const stripTabs = async () =>
  (
    await page.evaluate(() =>
      [
        ...document.querySelectorAll(
          '[role="tab"][data-testid="sortable-tab"]',
        ),
      ].map((tab) => ({
 ...
 */
export async function runSubagentTabGroupsAcceptance() {
  fixture = await mkdtemp(path.join(tmpdir(), "dg-606-"));
...
if (
  ...
  await runSubagentTabGroupsAcceptance();
}
