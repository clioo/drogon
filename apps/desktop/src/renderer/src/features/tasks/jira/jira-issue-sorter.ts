...
// src/renderer/src/components/jira-issue-sorter.ts and
// src/renderer/src/lib/locale-text-collators.ts — the tier weights, the
// per-site priority normalization and the sort semantics are the fork's;
// the Jira types are this repo's jira-contract shapes.
import type { JiraIssue, JiraPriority } from "../../../../../shared/jira-contract";

export type JiraIssueSortColumn =
  | "key"
  | "title"
  | "status"
  | "priority"
...
function compareNumericLocaleText(a: string, b: string): number {
...
export function sortJiraIssues(
  ...
  return [...issues].sort((a, b) => {
    ...
    if (orderBy === "key") {
      ...
      const weightA = getJiraPriorityWeight(
        a.priority?.name,
        a.priority?.id,
        prioritiesA,
      );
      const weightB = getJiraPriorityWeight(
        b.priority?.name,
        b.priority?.id,
        prioritiesB,
      );
