...
// src/renderer/src/components/github/IssueSourceSelector.tsx. Adaptations:
// GitHubOwnerRepo is the local { owner, repo } shape (the daemon's
// `tasks.remotes` only ever resolves github.com slugs, so the source's
// GHES host identity collapses to a case-insensitive slug compare), and
// the i18n translate() calls carry their English defaults literally like
// the other ported task-page components.
import type { JSX } from "react";
import { cn } from "./cn";
import { Tooltip, TooltipContent, TooltipTrigger } from "./ui/tooltip";

/** GitHub owner/repo identity as `tasks.remotes` reports it. */
export type GitHubOwnerRepo = { owner: string; repo: string };

/** The persisted per-repo pin; `undefined` is the fork's `'auto'`
 *  (storage leaves the key off for never-touched repos). */
export type IssueSourcePreference = "upstream" | "origin";
...
export type IssueSourceSelectorProps = {
  /** The repo's persisted preference (`undefined` renders as `'auto'`). */
  ...
  disabled?: boolean;
  className?: string;
  /** `'compact'` strips text from the pills and shows just "U" / "O".
   *  Defaults to `'labeled'` on the Tasks header. */
  density?: "labeled" | "compact";
  /** Suppresses the "Issues from <slug>" hover tooltip. */
...
function segmentClass(state: PillState, disabled: boolean | undefined): string {
  return cn(
    // Why: segments live *inside* an outer chip (see `issueSourceChipClass`
    // below) so they deliberately carry no border of their own — a second
    // border here would double-stroke the chip outline and look heavy.
    // Active state is expressed by a slightly darker inner background that
    // sits one step above the chip's own `bg-muted/40`.
...
// Why: exported so the Tasks-header row can wrap the selector in the same
// pill shape used by the static indicator. Keeping the styling here means
// the chip and its segments stay visually consistent.
 ...
 *   - `upstream` null (no upstream remote configured): the heuristic
 *     already resolves to origin and any click would be a no-op.
 ...
 * explicit choice*, not a visual state the user would click. It's
 * expressed by highlighting whichever pill the heuristic currently
 * resolves to. Any click writes the explicit preference so later
 * remote-topology changes cannot silently move the effective source.
 */
export function IssueSourceSelector({
...
}: IssueSourceSelectorProps): JSX.Element | null {
  if (!origin || !upstream) {
    return null;
  }
  ...
  // Why: in `'auto'`/unset, the effective pill is whatever the daemon's
  // upstream-first heuristic picks. Since we only render here when
  ...
  // Why: "pin-on-click" semantics — any click writes the explicit
  // preference, even when the pill is already active under `auto`.
  // Short-circuiting when the clicked pill already looks selected would
  // leave `preference === undefined`, which means a later remote-topology
  // change (upstream removed or re-added) could silently move the
  // effective source. Only short-circuit when the persisted preference
  // already matches the click.
