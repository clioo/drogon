...
// Adapted: this build has one xterm instance per TerminalPane; the live
// setting callback replaces PaneManager's settings selector.
...
import type { TerminalTuiMouseWheelDistanceState } from "./terminal-tui-wheel-reports";
...
function resolveTerminalWheelCellHeight(
...
export function attachTerminalMouseWheelMultiplier(
  ...
  terminal.attachCustomWheelEventHandler((event) => {
    ...
    const target =
      event.currentTarget instanceof EventTarget
        ? event.currentTarget
        : terminal.element;
    if (!target) {
      return true;
