/* MIT Copyright (c) 2026 Lovecast Inc. Ported verbatim from Orca
   src/renderer/src/lib/palette-match/match-field.ts at pinned source c9790628
...
import {
  isPaletteTypoCandidate,
  isWithinOnePaletteEdit,
} from "./typo-distance";
...
const SHORT_TOKEN_QUALITIES: ReadonlySet<PaletteMatchQuality> =
  new Set<PaletteMatchQuality>([
    "field-exact",
    "word-exact",
    "field-prefix",
    "word-prefix",
...
const PREFIX_QUALITIES: ReadonlySet<PaletteMatchQuality> =
  new Set<PaletteMatchQuality>(["field-prefix", "word-prefix"]);
...
function allowedQualities(
  ...
  if (token.isSingleLatinCharacter) {
    qualities = qualities.filter((quality) =>
      SHORT_TOKEN_QUALITIES.has(quality),
    );
  }
...
function matchAtomComponentRun(
  atom: PaletteAtom,
  compact: string,
): [number, number] | null {
...
function matchLiteral(
  ...
  if (qualities.has("field-exact") && normalized === text) {
    return {
      quality: "field-exact",
      ranges: toRanges(field, 0, normalized.length),
    };
  }
  if (qualities.has("word-exact")) {
    ...
    if (word) {
      return {
        quality: "word-exact",
        ranges: toRanges(field, word.start, word.end),
      };
    }
    const atom = field.atoms.find(
      (entry) => normalized.slice(entry.start, entry.end) === text,
    );
    if (atom) {
      return {
        quality: "word-exact",
        ranges: toRanges(field, atom.start, atom.end),
      };
    }
  }
  ...
  if (qualities.has("word-prefix")) {
    ...
    const atom = field.atoms.find((entry) =>
      normalized.startsWith(text, entry.start),
    );
    const start =
      word && atom
        ? Math.min(word.start, atom.start)
        : (word?.start ?? atom?.start);
    if (start !== undefined) {
      return {
        quality: "word-prefix",
        ranges: toRanges(field, start, start + text.length),
      };
    }
  }
...
function matchTypo(
  ...
  if (
    !qualities.has("typo") ||
    !token.isLetterOnly ||
    !isPaletteTypoCandidate(token.text)
  ) {
    return null;
  }
