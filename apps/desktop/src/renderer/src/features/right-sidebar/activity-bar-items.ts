/* MIT Copyright (c) 2026 Lovecast Inc. Ported from Orca's
   ...
   store; mentu/vault/worktrees/pr-checks/checks/ports/plugin entries are
   out of MVP scope, and the third entry hosts this repo's session-details
   inspector content, which has no source equivalent. */
import { Files, GitBranch, Info } from "lucide-react";
...
/** Source order for the MVP-relevant entries: Explorer, Source Control. */
...
export function getVisibleRightSidebarActivityItems(
  ...
  state: { gitAvailable: boolean },
): ActivityBarItem[] {
  return items.filter((item) => !item.gitOnly || state.gitAvailable);
}
