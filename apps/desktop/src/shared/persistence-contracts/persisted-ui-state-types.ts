...
// (SHA256 b7162f2c1982b2353d4cc6db11c68077909236546e3414509e96f3c057434244).
// Wholesale type port with one provenance-marked reuse: the source imports
// ReleaseChannel from src/shared/release-channel.ts
// (SHA256 of the pinned file recorded in the mapping report); the accepted
// candidate ../update-status-types declares the identical union
// 'stable' | 'rc' | 'hourly' | 'daily' | 'adhoc', verified structurally
// equivalent against the pinned declaration, so it is imported rather than
// duplicated.

import type { ReleaseChannel } from '../update-status-types'
...
import type { MentuSessionStatesByKey } from './mentu-session-state-types'
...
export type PersistedUIState = {
  ...
  /** Client-owned Mentu selection and unsaved source projections, keyed by execution host/workspace/session. */
  mentuSessionStatesByKey?: MentuSessionStatesByKey
