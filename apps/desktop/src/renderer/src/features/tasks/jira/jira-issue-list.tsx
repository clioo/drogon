...
// src/renderer/src/components/task-page-jira-issue-list.tsx — row anatomy,
// grouping, collapse behavior, tooltips, keyboard access and the
// responsive grid classes are the fork's; the Jira types are this repo's
// jira-contract shapes (JiraProjectStatusOrder is the fork's shape, inlined
// because this repo's contract has no status-order RPC — the page passes
// null and the fork's alphabetical fallback orders the sections).
...
function statusColumnRanks(
  order: JiraProjectStatusOrder | null,
): Map<string, number> {
...
export function groupJiraIssuesByStatus(
  ...
  const sectionRanks = new Map(
    [...sections.values()].map((section) => [
      section.key,
      sectionColumnRank(section, ranks),
    ]),
  );
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button
              ...
              onClick={(event) => {
                event.stopPropagation();
                void windowShellOpenUrl(issue.url);
              }}
...
/** The row's external-link opener: the fork's `window.api.shell.openUrl`. */
