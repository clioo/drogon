...
- `packaged-runtime-admission.test.mjs`, `desktop-artifacts.mjs` (+ test) —
  sealed-bundle identity v3 (final seal format referenced by the integrated
  acceptance: 293 files / 307,543,311 bytes seal; legacy metadata is not
  install authority).
...
Pending / not yet exact:

- **Final packaged notices are unproven.** Notices are regenerated at
  package time by `package-notices.mjs` from live pnpm/cargo state; nothing
  yet inspects the notices actually shipped inside a built bundle. The
  special-case pin for `react-remove-scroll-bar@2.3.8` shows the generator
  already carries hand-maintained text.
- The source's adjacent OFL notice next to the Nerd font is NOT a faithful
...
- Complete installed-dependency census: only 4 packages + font components
  have exact evidence; the remaining dependency set and publisher identity
  are unattested.
