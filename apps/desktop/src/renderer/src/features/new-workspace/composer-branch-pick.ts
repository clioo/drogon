/* MIT Copyright (c) 2026 Lovecast Inc. Ported from Orca's
   src/shared/composer-branch-selection.ts (the smart field's branch-pick
   semantics: auto-name, reuse eligibility and the branch-name override).
   The push-target/fork-warning resolution stays unported — Drogon has no
   push-target picker in the composer. */
...
export function isBranchCheckedOutInWorktrees(
  ...
  return worktreeBranches.some(
    (ref) => ref.replace(/^refs\/heads\//, "") === branchName,
  );
}

export function getComposerRepoWorktreeBranches(
  worktrees: readonly { branch: string }[],
): string[] {
  return worktrees.map((worktree) => worktree.branch);
}

/**
 * Decide whether a picked branch row is an existing LOCAL branch that can be
 * reused (checked out) instead of branched off, and whether reuse should
 * default ON.
 *
 ...
 * Reuse defaults ON only when the worktree name was auto-derived from the
 * branch (the selection produced a branch-name override); a user who typed a
 * custom worktree name first is branching off the ref, so reuse stays OFF
 * unless they opt in.
 */
...
/**
 * The branch-name override to apply for a picked branch. A local branch
 * already checked out in another worktree can't be reused, so it must NOT be
 * pinned as the override — pinning it would collide and silently produce a
 * suffixed branch. In that case fall back to letting the worktree name
 * derive a fresh branch from the selected ref as base; otherwise use the
 * selection's override unchanged.
 */
