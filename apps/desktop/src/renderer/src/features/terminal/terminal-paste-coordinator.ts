...
// Adapted: only the synchronous `planTerminalPaste` survives (no WithYield
// consumer in this repo); the mode-choice, metadata and newline policy are
...
import { measurePastePayloadMetadata } from './terminal-paste-payload-metadata'
import type {
...
export { executeTerminalPastePlan, getTerminalPasteOperationTimeoutMs } from './terminal-paste-executor'
...
export function createTerminalPastePayload({
  ...
  const metadata = measurePastePayloadMetadata(text, { stopAfterBytes: maxBytes })
  return {
    plainText: text,
    source,
