/* MIT Copyright (c) 2026 Lovecast Inc. Ported from Orca's
   src/renderer/src/components/new-workspace/ProjectComboboxRow.tsx
   (adapter: RepoBadgeMark from components/repo/RepoBadgeLabel.tsx is inlined
   — Drogon projects carry no badge color, so the mark renders the source's
   colorless square). */
...
/** The source's RepoBadgeMark: a square color dot, colorless when unset. */
function ProjectBadgeMark({
  color,
  className,
}: {
...
export function ProjectOptionMark({
  ...
  return option.kind === "project-group" ? (
    ...
    <ProjectBadgeMark color={option.badgeColor} />
  );
}
