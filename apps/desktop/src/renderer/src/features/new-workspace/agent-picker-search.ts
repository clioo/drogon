/* MIT Copyright (c) 2026 Lovecast Inc. Ported from Orca's
   src/renderer/src/lib/agent-picker-search.ts (adapter: the entry type is
   this repo's { id, label, cmd } composer agent option, and the clipboard
   byte-limit helper is inlined). */
...
type RankedAgent<T extends AgentPickerEntry> = {
  agent: T;
  score: number;
  index: number;
};
...
export function getAgentPickerCommandValue({
  ...
  currentValue: AgentPickerEntry["id"] | null;
  filteredAgents: readonly AgentPickerEntry[];
  rawQuery: string;
}): string {
...
export function searchAgentPickerEntries<T extends AgentPickerEntry>(
  agents: readonly T[],
  rawQuery: string,
): T[] {
  const query = getAgentPickerSearchQuery(rawQuery);
...
function scoreAgent(agent: AgentPickerEntry, query: string): number {
  return Math.min(
