...
Recovery follow-up: four complete shared-control suites now add45 passing original cases; two prior liveness cases passed again. See shared-control-recovery-source-baseline.md/json for exact scopes, real-loopback versus simulated checks and remaining gates. This supersedes the historical statements below that reconnect suites remain unrun.
