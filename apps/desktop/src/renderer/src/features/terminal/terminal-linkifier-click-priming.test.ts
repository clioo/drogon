...
// src/renderer/src/components/terminal-pane/terminal-linkifier-click-priming.test.ts.
...
describe("installTerminalLinkifierClickPriming", () => {
  it("primes a fresh link before xterm snapshots a macOS mousedown", () => {
    macUserAgent();
    const order: string[] = [];
  ...
  it("primes every left click and preserves Shift for routing", () => {
    ...
    const { terminal, registrations } = createTerminal({
  ...
  it("primes plain action clicks and degrades safely when xterm internals are unavailable", () => {
    ...
    expect(() =>
      getMouseDownHandler(absent.registrations)(
        modifierMouseDown({ metaKey: true }),
      ),
    ).not.toThrow();
