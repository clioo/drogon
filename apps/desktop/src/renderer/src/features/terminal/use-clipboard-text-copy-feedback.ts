...
// src/renderer/src/hooks/use-clipboard-text-copy-feedback.ts. Adapted: the
// clipboard write goes through an injected `writeClipboardText` (Drogon has
// no `window.api.ui`); the feedback lifecycle is verbatim.
 ...
 */
export function useClipboardTextCopyFeedback(
  text: string,
  ...
  }, [canCopy, scheduleReset, text, writeClipboardText])
