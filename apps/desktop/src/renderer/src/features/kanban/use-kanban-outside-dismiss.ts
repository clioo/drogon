/* MIT Copyright (c) 2026 Lovecast Inc. Ported verbatim from Orca
   ...
   the keep-open selector covers the Radix data-slot markers Drogon's ui kit
   emits too. */
...
export function isWorkspaceBoardKeepOpenTarget(
...
export function useWorkspaceKanbanOutsideDismiss(params: {
  ...
  useEffect(() => {
    ...
    const handlePointerDown = (event: PointerEvent): void => {
      const content = boardRef.current?.closest<HTMLElement>(
