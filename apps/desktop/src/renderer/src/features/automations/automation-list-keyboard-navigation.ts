...
// src/renderer/src/components/automations/automation-list-keyboard-navigation.ts.
// Literal port: local-only MVP rows still carry `kind: "local"` so the
// navigation helpers stay structurally identical to the source.
export type AutomationListViewItem = { kind: "local"; id: string };

export type AutomationListArrowKey = "ArrowUp" | "ArrowDown";

export function isAutomationListArrowKey(
  key: string,
): key is AutomationListArrowKey {
...
export function findAutomationListSelectionIndex(
  ...
  if (selectedId != null) {
    return items.findIndex(
      (item) => item.kind === "local" && item.id === selectedId,
    );
  }
  return -1;
}
