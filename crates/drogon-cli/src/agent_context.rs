...
pub fn all_commands() -> Vec<AgentCommand> {
    let mut commands = vec![
        ...
        entry(
            ...
            "drogon-cli worktree create --project <ID> --name <NAME> [--base <REF>] [--parent <ID> | --no-parent]",
            &[
                "base",
                "base-branch",
                "name",
            ...
            &[
                ...
                "drogon-cli worktree create --project proj-1 --name fix-a --parent ws-9 --json",
            ],
            &[
                "--base-branch is an alias of --base: the new branch starts at that ref, otherwise at the project's HEAD.",
                "Run from inside a Drogon session, the new worktree nests under the calling session's worktree (DROGON_WORKSPACE_ID), or else under the project worktree that contains the current directory: the sidebar shows it as that worktree's child, the way a subagent belongs under its coordinator.",
                "--parent <ID> names the parent explicitly (a worktree id or workspaceId of the same project, as listed by `worktree list --json`); --no-parent creates a top-level worktree. An explicit parent that is not in the project refuses the create.",
        ...
        entry(
            ...
            &[
                ...
                "Every other byte is delivered unchanged, so an LF-separated multi-line message lands whole and only the final Return submits it.",
                "A carriage return INSIDE the value is Enter too: convert CRLF line endings to LF first, or each CR submits early.",
                "The Return is delivered as its own keypress after the body, and for a far end that reads a paste as text the body is wrapped in paste markers first, so a paste-detecting TUI cannot swallow the Return mid-turn.",
                "Reads a paste as text means bracketed paste is on AND the far end is either a harness Drogon launched or a program on the normal screen; a full-screen application in a plain session (vim, less, nano) is never framed, because a paste there is text in a buffer rather than the keystrokes you meant. The result's bracketedPaste says which happened.",
                "The result adds submittedEnter, true when that Return reached the PTY, and enterDelivery saying how: keypress (its own write), inline (fused into the body's burst, which a paste-detecting TUI may not submit) or none. acceptedBytes counts what reached the PTY, so a trailing CRLF reports one byte fewer than the input.",
                "Neither field claims the agent submitted a turn: confirm that by reading the session (terminal read, or terminal wait --for output), never by trusting the send alone.",
        ...
        entry(
            ...
            &[
                ...
                "For issues, use `drogon-cli bot watch-issue` — the same guarantees on the repository's issues.",
            ],
        ),
        entry(
            "bot watch-issue",
            &["bot", "watch-issue"],
            "Watch a GitHub repository for the ISSUES you name and release the Bot's action for each new one; the released session opens a worktree in the project with the harness and skills the watch names",
            "drogon-cli bot watch-issue --bot <ID> --workspace <ID> --repo <OWNER/NAME> [--filter opened|assigned] [--login <LOGIN>] [--harness <ID>] [--skill <NAME>]... [--secret-ref <REF>] [--api-base <URL>] [--cron <EXPR> | --manual] [--disabled] [--approve] [--responsibility-id <ID> | --responsibility-name <NAME> [--instructions <TEXT>]]",
            &[
                "api-base",
                "approve",
                "bot",
            ...
            &[],
            &[
                "drogon-cli bot watch-issue --bot bot-1 --workspace ws-1 --repo clioo/drogon --filter opened --secret-ref GITHUB_TOKEN_REF --responsibility-name 'Triage new issues' --instructions 'Reproduce it, fix it, open a PR and close the issue.' --approve --json",
                "drogon-cli bot watch-issue --bot bot-1 --workspace ws-1 --repo clioo/drogon --filter assigned --login clioo --harness codex --skill drogon-cli --json",
            ],
            &[
                "Requires the service capability bot.self.v1.",
                "This is the USER lane (bot.monitor_create): the rule's project is the project workspace the Bot lives in, so the released session opens a worktree of that project.",
                "Staged parked unless --approve is passed; approval arms the EXACT rule hash (repo, filter, login, harness, skills, secret refs are all inside it).",
                "The token is named by reference only (--secret-ref). Grant it with `drogon-cli bot grant-secret` and store it with `drogon-cli secrets set --kind github`.",
                "The same issue never fires twice, and a watch that was not running seeds its baseline instead of replaying every issue that is already open. Never hand-roll this with `bot create-automation`: that has no baseline, so its first run dispatches one session per open issue.",
                "Pull requests are excluded. GitHub's issue list returns them; this watch drops them, so a PR never releases an issue action.",
                "One issue releases one session at a time: a burst drains one per tick, and the per-bot daily delegation cap still applies.",
            ],
        ),
        entry(
            "bot bind-monitor",
        ...
        entry(
            "graph orchestrator-start",
            &["graph", "orchestrator-start"],
            "Start a daemon-owned workflow with a snapshot of the Subagent policy",
            "drogon-cli graph orchestrator-start --workspace <ID> --file <PATH|->",
            &["file", "workspace"],
            &[],
            &["drogon-cli graph orchestrator-start --workspace ws-1 --file main-task.json"],
            &[
                "The file contains one enabled GraphNodeIntent with no dependencies. The daemon runs the main task and then both evaluation roles when adversarial testing is enabled. Findings are evaluation results, not runtime failures. No desktop window is required.",
            ],
        ),
        entry(
            "graph orchestrator-status",
            &["graph", "orchestrator-status"],
            "Read the latest durable workflow, its policy snapshot and role attempts",
            "drogon-cli graph orchestrator-status --workspace <ID>",
            &["workspace"],
            &[],
            &["drogon-cli graph orchestrator-status --workspace ws-1 --json"],
            &["Status reads do not start or advance work. The daemon owns scheduling."],
        ),
        entry(
            "graph observability",
            &["graph", "observability"],
            "Read native Work Graph evidence and token measurements",
            "drogon-cli graph observability --workspace <ID>",
            &["workspace"],
            &[],
            &["drogon-cli graph observability --workspace ws-1 --json"],
            &[
                "Reads .drogon/evidence.json and .drogon/usage.json. Missing token fields stay unknown, never zero.",
            ],
        ),
        entry(
            "graph evidence-add",
            &["graph", "evidence-add"],
            "Append one lead-agent checkpoint to native Work Graph evidence",
            "drogon-cli graph evidence-add --workspace <ID> --summary <TEXT> [--status <STATUS>] [--detail <TEXT>] [--artifact <PATH>]",
            &[
                "agent",
                "artifact",
                "detail",
            ...
            &[],
            &[
                "drogon-cli graph evidence-add --workspace ws-1 --summary \"Unit tests pass\" --status progress --artifact reports/unit.txt",
            ],
            &[
                "Writes .drogon/evidence.json atomically. Use meaningful checkpoints, not per-tool-call noise.",
            ],
        ),
        entry(
            "graph usage-add",
            &["graph", "usage-add"],
            "Append one exact agent token measurement to native Work Graph usage",
            "drogon-cli graph usage-add --workspace <ID> [--input <N>] [--output <N>] [--cache-read <N>] [--cache-write <N>]",
            &[
                "agent",
                "cache-read",
                "cache-write",
            ...
            &[],
            &[
                "drogon-cli graph usage-add --workspace ws-1 --input 120 --output 30 --agent leader --harness pi",
            ],
            &[
                "At least one exact token field is required. Report incremental measurements once; omit values the harness did not provide.",
            ],
        ),
        entry(
            "graph orchestrator-stop",
            &["graph", "orchestrator-stop"],
            "Request cancellation of a durable workflow",
            "drogon-cli graph orchestrator-stop --workspace <ID> --run <ID>",
            &["run", "workspace"],
            &[],
            &["drogon-cli graph orchestrator-stop --workspace ws-1 --run run-1"],
            &["Poll orchestrator-status until stopped; stopping is not proof of exit."],
        ),
        entry(
            "graph orchestrator-resume",
            &["graph", "orchestrator-resume"],
            "Resume a confirmed stopped workflow",
            "drogon-cli graph orchestrator-resume --workspace <ID> --run <ID>",
            &["run", "workspace"],
            &[],
            &["drogon-cli graph orchestrator-resume --workspace ws-1 --run run-1"],
            &["Completed roles are retained. Unverifiable work is not automatically retried."],
        ),
        entry(
            "graph resume",
            &["graph", "resume"],
...
fn work_commands() -> Vec<AgentCommand> {
    ...
    vec![
        entry(
            "work board",
            &["work", "board"],
            "Show a Work board: columns in order with their tickets, linked sessions and prompt triggers",
            "drogon-cli work board [--project <PROJECT>] [--board <BOARD>] [--sprint <SPRINT|active|backlog>]",
            &["board", "project", "sprint"],
            &[],
            &[
                "drogon-cli work board --json",
                "drogon-cli work board --project Drogon",
                "drogon-cli work board --board 'Platform Delivery' --sprint backlog",
            ],
            &[
                REQUIRES,
                "--project accepts a project id or its name. Without --board it is My work; --board/--sprint need work.boards.v1.",
                "On an imported scrum board the view is one sprint (default: the active one) or the backlog; a closed sprint is read-only and its reply carries view.outcome (completed / carried / backlog).",
            ],
        ),
        entry(
            "work boards",
            &["work", "boards"],
            "List the Work boards: My work and every imported board, with unsynced moves",
            "drogon-cli work boards",
            &[],
            &[],
            &["drogon-cli work boards --json"],
        ...
        entry(
            "work import preview",
            &["work", "import", "preview"],
            "Show a provider board's columns, sprints and issues before importing",
            "drogon-cli work import preview --board <ID> [--scope <board|backlog|sprint:ID>] [--assignee <me|none|ID>] [--project <NAME|none>] [--status <ID>] [--search <TEXT>] [--open] [--provider <PROVIDER>] [--site <SITE>]",
            &[
                "assignee", "board", "open", "project", "provider", "scope", "search", "site",
                "status",
            ],
            &[],
            &[
                "drogon-cli work import preview --board 7 --scope sprint:25 --json",
                "drogon-cli work import preview --provider linear --board <TEAM_ID> --assignee me --open",
            ],
            &[
                BOARDS,
                "Issues already on the Drogon board carry importedTicketId. `me` is the connected account; `facets` counts people, projects and statuses over every issue (their ids feed --assignee and --status).",
                "Rows carry no descriptions; a board past one reply's budget is cut (`truncated`, `total`): narrow it with the filters.",
            ],
        ),
        entry(
        ...
        entry(
            "work import remove",
            &["work", "import", "remove"],
            "Remove an imported board and its tickets from Drogon (the provider is untouched)",
            "drogon-cli work import remove --board <BOARD>",
            &["board"],
            &[],
            &["drogon-cli work import remove --board 'Platform Delivery'"],
            &[BOARDS, "Linked sessions keep running."],
        ),
        entry(
            "work import run",
            &["work", "import", "run"],
            "Import a provider board and the chosen issues (or all of them)",
            "drogon-cli work import run --board <ID> (--issue <KEY> ... | --mine | --all) [--auto-import-mine <true|false>] [--project <PROJECT>] [--provider <PROVIDER>] [--site <SITE>]",
            &[
                "all",
                "auto-import-mine",
                "board",
            ...
            &[
                ...
                "drogon-cli work import run --board 9 --all",
            ],
            &[
                BOARDS,
                "The first import creates the board's columns from the provider's, each mapped to its statuses; later imports add the chosen issues and refresh ones already in.",
                "Imported tickets show the provider key (APP-128); the Drogon key stays in `key`. --project names the Drogon project whose workspace their sessions start in.",
            ],
        ),
        entry(
            "work sync",
            &["work", "sync"],
            "Read an imported board's provider now (it also syncs every 5 minutes)",
            "drogon-cli work sync --board <BOARD>",
            &["board"],
            &[],
            &["drogon-cli work sync --board 'Platform Delivery'"],
            &[
                BOARDS,
                "The provider wins for title, description, type, priority, assignee and sprint. A status change moves a card with no pending move to the mapped column (logged as Moved by Jira, firing the column's on-enter prompt in the active sprint); with a pending move it is a conflict (see `work ticket resolve`); an unmapped status leaves the card in place; an issue gone from the provider is flagged, never deleted.",
            ],
        ),
        entry(
            "work push",
            &["work", "push"],
            "Push unsynced moves (status and sprint) to the provider: one ticket's or a whole board's",
            "drogon-cli work push (--board <BOARD> | --ticket <TICKET>)",
            &["board", "ticket"],
            &[],
            &[
                "drogon-cli work push --ticket APP-128",
                "drogon-cli work push --board 'Platform Delivery'",
            ],
            &[
                BOARDS,
                "A move into a mapped column is Drogon-only until pushed. A refusal (no workflow transition, closed sprint, issue gone) keeps the card where it is and records the provider's error on it.",
            ],
        ),
        entry(
            "work ticket new-session",
            &["work", "ticket", "new-session"],
            "Start a new session in a ticket's workspace, linked to the ticket",
            "drogon-cli work ticket new-session --ticket <TICKET> [--harness <HARNESS>] [--prompt <TEXT>]",
            &["harness", "prompt", "ticket"],
            &[],
            &["drogon-cli work ticket new-session --ticket APP-128 --harness claude"],
            &[
                BOARDS,
                "Default harness: the ticket column's, else the default agent.",
            ],
        ),
        entry(
            "work ticket rename-session",
            &["work", "ticket", "rename-session"],
            "Name a linked session on a ticket (an empty title restores the session's own)",
            "drogon-cli work ticket rename-session --ticket <TICKET> --session <ID> --title <TITLE>",
            &["session", "ticket", "title"],
            &[],
            &[
                "drogon-cli work ticket rename-session --ticket APP-128 --session 3f1c… --title Implement",
            ],
            &[BOARDS],
        ),
        entry(
        ...
        entry(
            "work ticket sprint",
            &["work", "ticket", "sprint"],
            "Carry a ticket over to the active sprint, send it to the backlog, or put it in an open sprint",
            "drogon-cli work ticket sprint --ticket <TICKET> --to <active|backlog|SPRINT>",
            &["ticket", "to"],
            &[],
            &["drogon-cli work ticket sprint --ticket APP-122 --to active"],
            &[
                BOARDS,
                "Unsynced until pushed (`work push`). Prompts reach only tickets in the active sprint; closed sprints take no issues.",
            ],
        ),
        entry(
            "work column create",
            &["work", "column", "create"],
            "Add a column to the Work board",
            "drogon-cli work column create --name <NAME> [--board <BOARD>] [--icon <ICON>] [--index <N>]",
            &["board", "icon", "index", "name"],
            &[],
            &["drogon-cli work column create --name Blocked --icon blocked --index 2"],
            &[
                REQUIRES,
                "Icons: backlog, todo, in_progress, review, qa, done, blocked. A fresh board starts with To do, In progress, Review, QA and Done.",
            ],
        ),
        entry(
            "work column delete",
            &["work", "column", "delete"],
            "Delete a column, moving its tickets to another one",
            "drogon-cli work column delete --column <COLUMN> [--move-to <COLUMN>]",
            &["column", "move-to"],
            &[],
            &["drogon-cli work column delete --column Blocked --move-to 'To do'"],
            &[
                REQUIRES,
                "A column that still holds tickets is refused without --move-to; the last column cannot be deleted.",
            ],
        ),
        entry(
            "work column list",
            &["work", "column", "list"],
            "List the columns with their prompt, triggers and recipients",
            "drogon-cli work column list [--board <BOARD>]",
            &["board"],
            &[],
        ...
        entry(
            "work column preview",
            &["work", "column", "preview"],
            "Show exactly what a send would type and to which sessions, changing nothing",
            "drogon-cli work column preview --column <COLUMN> [--ticket <TICKET>]",
            &["column", "ticket"],
            &[],
            &["drogon-cli work column preview --column Review --json"],
            &[
                REQUIRES,
                "Each recipient says what would happen: send (live), resume (no longer live) or start (nothing to resume).",
            ],
        ),
        entry(
            "work column send",
            &["work", "column", "send"],
            "Send a column's prompt now to every ticket in it, or to one ticket",
            "drogon-cli work column send --column <COLUMN> [--ticket <TICKET>] [--message <TEXT>]",
            &["column", "message", "ticket"],
            &[],
            &[
                "drogon-cli work column send --column Review",
                "drogon-cli work column send --column QA --ticket DRG-35 --message 'Re-run the smoke test'",
            ],
            &[
                REQUIRES,
                "Live linked sessions are typed into right away (framed body, then a separate Return). A session that is no longer live is resumed with the prompt; with nothing to resume a fresh session starts, and the replacement takes the old one's place on the ticket.",
                "A ticket with no linked session gets a new session in its workspace (or its project's) using the column's harness or the default agent.",
                "Every send is recorded: see `work sends`.",
            ],
        ),
        entry(
            "work column update",
            ...
            &[],
            &[
                "drogon-cli work column update --column Blocked --statuses Blocked",
                "drogon-cli work column update --column Review --on-enter true --pr-watch true --schedule 15m --message 'Review {ticket.pr} for {ticket.id}. After GitHub confirms the merge, run: drogon-cli work ticket move --ticket {ticket.id} --column QA'",
                "drogon-cli work column update --column QA --recipients primary --harness pi",
            ],
            &[
                REQUIRES,
                "Triggers combine: --on-enter (a ticket moved or created into the column), --schedule (15m, 2h, 1d or a UTC cron; every ticket in the column), --pr-watch (the ticket's PR changed state, review, checks or updated; polled every 5 minutes through gh).",
                "Placeholders: {ticket.id} {ticket.title} {ticket.pr} {ticket.pr_number} {ticket.url} {ticket.description} {ticket.next} {ticket.project} {ticket.column} {ticket.status} {ticket.drogon_key}. On an imported ticket {ticket.id} is the provider key (APP-128).",
                "--statuses (imported boards) maps provider statuses to the column; a status belongs to one column, and cards whose status was unmapped move in.",
                "Recipients: all linked sessions, or primary (the first linked one).",
            ],
        ),
        entry(
            "work sends",
            &["work", "sends"],
            "Delivery history: what each column typed into which session, and why",
            "drogon-cli work sends [--column <COLUMN>] [--ticket <TICKET>] [--limit <N>]",
            &["column", "limit", "ticket"],
            &[],
            &["drogon-cli work sends --ticket DRG-42 --json"],
            &[
                REQUIRES,
                "Triggers: enter, schedule, pr_change, manual. Each result is sent, resumed, started, skipped or failed (with the reason).",
            ],
        ),
        entry(
            "work ticket create",
            &["work", "ticket", "create"],
            "Create a ticket (its own key, e.g. DRG-41), optionally with a PR, a source link and sessions",
            "drogon-cli work ticket create --title <TITLE> [--project <PROJECT>] [--workspace <ID>] [--column <COLUMN>] [--description <TEXT>] [--pr <URL|#N>] [--source <URL>] [--next <TEXT>] [--session <ID> ...]",
            &[
                "column",
                "description",
                "next",
            ...
            &[],
            &[
                "drogon-cli work ticket create --title 'Improve Jira resume' --project Drogon --column Review --pr 648 --source https://jira.example.com/browse/DRG-9 --session \"$DROGON_SESSION_ID\"",
            ],
            &[
                REQUIRES,
                "Keys follow the project name (Drogon → DRG-n; no project → WRK-n). Creating a ticket into a column with an on-enter prompt sends it.",
            ],
        ),
        entry(
            "work ticket delete",
            ...
            "drogon-cli work ticket delete --ticket <TICKET>",
            &["ticket"],
            &[],
            &["drogon-cli work ticket delete --ticket DRG-41"],
            &[REQUIRES],
        ),
        entry(
            "work ticket link",
            &["work", "ticket", "link"],
            "Attach a session to a ticket so its column prompts reach it",
            "drogon-cli work ticket link --ticket <TICKET> --session <ID>",
            &["session", "ticket"],
            &[],
            &["drogon-cli work ticket link --ticket DRG-42 --session \"$DROGON_SESSION_ID\""],
            &[
                REQUIRES,
                "Inside a Drogon session, $DROGON_SESSION_ID names the session you are running in.",
            ],
        ),
        entry(
            "work ticket list",
            ...
            "drogon-cli work ticket list [--column <COLUMN>] [--project <PROJECT>] [--board <BOARD>] [--sprint <SPRINT>]",
            &["board", "column", "project", "sprint"],
            &[],
        ...
        entry(
            ...
            "drogon-cli work ticket move --ticket <TICKET> --column <COLUMN> [--index <N>] [--sprint <SPRINT>]",
            &["column", "index", "sprint", "ticket"],
            &[],
            &["drogon-cli work ticket move --ticket DRG-42 --column QA"],
            &[
                REQUIRES,
                "The reply carries `delivery`: the rendered prompt and what happened for each session.",
                "On an imported board a move into a mapped column is Drogon-only (sync: pending) until `work push`; --sprint names the viewed sprint, and a closed one refuses the move.",
            ],
        ),
        entry(
            "work ticket open",
            &["work", "ticket", "open"],
            "Open a ticket's session, resuming it when it is no longer live",
            "drogon-cli work ticket open --ticket <TICKET> --session <ID>",
            &["session", "ticket"],
            &[],
            &["drogon-cli work ticket open --ticket DRG-42 --session 3f1c…"],
            &[
                REQUIRES,
                "A resumed (or freshly started) replacement takes the old session's place on the ticket.",
            ],
        ),
        entry(
            "work ticket show",
            ...
            "drogon-cli work ticket show --ticket <TICKET>",
            &["ticket"],
            &[],
        ...
        entry(
            ...
            "drogon-cli work ticket unlink --ticket <TICKET> --session <ID>",
            &["session", "ticket"],
            &[],
            &["drogon-cli work ticket unlink --ticket DRG-42 --session 3f1c…"],
            &[REQUIRES],
        ),
        entry(
            "work ticket update",
            &["work", "ticket", "update"],
            "Edit a ticket; `none` clears --project, --workspace, --pr or --source",
            "drogon-cli work ticket update --ticket <TICKET> [--title <TITLE>] [--description <TEXT>] [--project <PROJECT|none>] [--workspace <ID|none>] [--pr <URL|#N|none>] [--source <URL|none>] [--next <TEXT>]",
            &[
                "description",
                "next",
                "pr",
            ...
            &[],
            &[
                "drogon-cli work ticket update --ticket DRG-42 --pr https://github.com/clioo/drogon/pull/663",
            ],
            &[REQUIRES],
        ),
    ]
