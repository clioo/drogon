...
export function stageCapsule({ manifestPath, sourceRoot, repoRoot = REPO_ROOT, label }) {
  ...
  writeFileSync(configPath, GENERATED_VITEST_CONFIG, { flag: "wx" });
...
function assertOwnedStageReceipt(receipt, repoRoot) {
  ...
  for (const field of ["capsuleId", "sourceRevision", "manifestSha256", "entryTestFile", "capsuleRoot", "staged", "license", "expectedTestCounts"]) {
...
export function executeCapsule({
  ...
  if (readFileSync(configPath, "utf8") !== GENERATED_VITEST_CONFIG) {
...
async function main() {
  ...
  const report = {
    disclaimer:
      ...
      "result. It validates a single, pinned pure-function capsule in " +
