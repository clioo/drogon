...
- **Run the work from a board.** Work keeps Drogon tickets (`DRG-41`) on columns you configure. Link a ticket to the issue it tracks elsewhere, its pull request and the sessions working on it; a column types its prompt into those sessions when a ticket enters it, on a schedule, or when the PR changes — resuming a session that stopped, or starting one when there is nothing to resume. Click a linked session to jump to it. Import a Jira board to work its sprints the same way: pick the board, choose the issues, and its columns map to Jira's statuses. Cards keep their Jira key, type, priority and assignee; a card you move waits "Not synced to Jira" until you push it, Jira's own changes move cards on sync (every 5 minutes, or on demand), prompts fire only in the active sprint, and a closed sprint stays a read-only record of what was completed, carried forward or returned to the backlog. Everything is scriptable with `drogon-cli work`.
