...
// Adapted: ORCA_BROWSER_BLANK_URL ('data:text/html,') is
// DROGON_BROWSER_BLANK_URL ('about:blank') throughout.
import { describe, expect, it } from "vitest";
import { DROGON_BROWSER_BLANK_URL } from "./browser-url";
import {
  buildSearchUrl,
...
describe("browser-url helpers", () => {
  ...
  it("keeps normal web URLs and blank tabs in the allowed set", () => {
    ...
    expect(normalizeBrowserNavigationUrl("")).toBe(DROGON_BROWSER_BLANK_URL);
    expect(normalizeBrowserNavigationUrl("about:blank")).toBe(DROGON_BROWSER_BLANK_URL);
  });
  ...
  it("normalizes pasted absolute local paths to file URLs", () => {
    ...
    expect(normalizeBrowserNavigationUrl("\\\\wsl.localhost\\Ubuntu\\home\\me\\Example.ipynb")).toBe(
      "file://wsl.localhost/Ubuntu/home/me/Example.ipynb",
    );
  });
