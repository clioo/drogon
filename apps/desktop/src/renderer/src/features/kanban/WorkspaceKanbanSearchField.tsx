/* MIT Copyright (c) 2026 Lovecast Inc. Ported from Orca
   src/renderer/src/components/sidebar/WorkspaceKanbanSearchField.tsx at
   pinned source c9790628 (clioo/drogon-orca). Adaptations: i18n
   translate(id, fallback) collapsed to English fallback strings; Button/Input
   imported from Drogon's shadcn ui kit (same recipes). Structure, classes,
   overlay reserve math, Escape/IME handling and ARIA are the source's. */
...
function formatAnnouncement(matchCount: number, totalCount: number): string {
  return matchCount === 0
    ? "No workspaces match"
    : `${matchCount} of ${totalCount} workspaces match`;
}
...
export default function WorkspaceKanbanSearchField({
  ...
  const tooLargeMessage = isTooLarge
    ...
    : null;
  const tooLargeLabel = isTooLarge ? "Too long" : null;
  ...
  return (
    <div className="relative flex min-w-0 max-w-xs flex-1 items-center">
      ...
      <Input
        ...
        style={
          hasText ? { paddingRight: overlayReserve(badgeText) } : undefined
        }
