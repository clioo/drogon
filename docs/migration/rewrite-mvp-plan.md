...
| Shell R14-B: menú nativo de la app con submenú Appearance, persistencia de bounds de ventana, badge del dock | R14 | GLM 5.3 Flash | fusionada: menú del fuente con hints de chords desde la tabla compartida (keybindings movidos a shared/), window-state.json con guardas del fuente, badge 99+; 1800 tests | #122 |
...
| J1 R16-M: pérdida y recuperación de la conexión con el daemon (banner, segmento de la barra de estado, overlay del panel, escalera de backoff del fuente, sin toast de reconexión) | R16 | Muse | fusionada: banner/segmento/overlay del fuente, escalera de backoff, sin toast de reconexión | #180 |
...
| QA fix R16-X: #144 el editor no recibe teclado; J2 editar/guardar de extremo a extremo con check sellado que teclea | R16 | Muse | fusionada (#210) y R16-X2 (#256: punto de sucio en la pestaña, cabecera del editor del fork; #207 cerrado con evidencia) | #210 #256 |
...
| J11 R16-Z: primer arranque del paquete instalado de extremo a extremo (data dir limpio, recursos, runtime Mentu, reinicio) | R16 | Muse | fusionada (#219: socket corto para data dirs profundos, hash del runtime Mentu tras firma ad-hoc, `--bundle` en drogon-ui.mjs); #201 icono en R16-Z2 (lanzada) | #219 |
| J3 R16-AA: drogon-cli de extremo a extremo desde una sesión (agent-context v1, project remove, alias de wait del fuente) | R16 | Muse | fusionada: agent-context v1, project remove, alias de wait del fuente; queda el tramo con Electron y modelo local | #200 |
| J5 R16-AC: paletas ⌘J/⌘P/⌘K y cambio de workspace/sesión de extremo a extremo | R16 | Muse | fusionada | #205 |
| QA fix R16-AE: #192 modelo del diálogo de lanzamiento, #194 derivación única del estado del agente, #197 filas New Markdown y Agent settings | R16 | Muse | fusionada: #192 #194 #197 cerrados | #206 |
| QA fix R16-T (plegado): #188 runs programados con la política de harness del bot | R16 | Muse | en R16-T | |
| QA fix R16-X (plegado): #195 #196 punto de sucio y cabecera del editor del fuente | R16 | Muse | en R16-X | |
...
| J10 R16-AD: Settings persisten y surten efecto de extremo a extremo (tema, harness por defecto, atajos, notificaciones, Git) | R16 | Muse | fusionada (#223: rebind de atajos, tipografía con un solo store) y #239 (#213 Restart daemon) | #223 #239 |
...
| J1 R16-AG: menú nativo, badge del Dock, notificaciones y estado de ventana de extremo a extremo | R16 | Muse | fusionada (#214) y R16-AG2 (#226: barra de estado sin desborde a 760 px, cierra #209) | #214 #226 |
| J1 R16-AH: superficie del terminal de extremo a extremo (salida larga, buscar, links, pegar, zoom, overlay de salida, restauración) | R16 | Muse | fusionada (#255: priming de clics en links, http → pestaña de browser, tema Ghostty/Tango del fork, BEL); #230 se verifica en R16-AR | #255 |
...
| Instalación: el bundle de main 66a772a falló la aceptación sellada (#209); /Applications conserva f0ce137 (sellado en verde) hasta que pase el nuevo sellado | R16 | Coordinador | instalado bundle 3783ecc (sellado 40/40) el 2026-09-08 02:0x: incluye lanzamiento directo de agentes con defaults de Orca, filas de sesión en la tarjeta, lista de sesiones tras reinicio, terminal e2e, editor parity |  |
...
| J2/J4 R16-AM: barra derecha de extremo a extremo (Explorer, Ports, Session details, rail) | R16 | Muse | fusionada (#282); restos en R16-BC fusionados (#304: #275 cwd de Open in terminal, Stop process en Ports, drag-and-drop del Explorer) | #282 #304 |
...
| QA fix R16-AR: #254 toggle de captura de enlaces, #187 aislamiento del entorno de Pi headless, #230 colores del TUI; pliega #273 (SIGINT heredado) | R16 | Kimi k3 | fusionada (#285 vía R16-AR3: #254 #187 #230 #273 cerrados; reset de señales en el hijo PTY antes de exec) | #285 |
...
| R16-AW: #264 cuelgue del renderer con Forward en el browser, #259 Restart daemon en el paquete | R16 | luna | fusionada (#288); R16-AW2 #279 guests locales siguen pintando (#291); R16-AW3 fusionada (#299: el segundo daemon vuelve a rechazar el data-dir sin esperar el arranque; #290 cerrado) | #288 #291 #299 |
| R16-AY: barra de estado J12 (medidores de uso, awake, contadores, clics) | R16 | GLM | fusionada (#281); restos en R16-AY2 fusionados (#297: popover de uso, CTA vacío, awake Auto, clics de segmento) | #281 #297 |
...
| R16-BE: #188 responsabilidades programadas y run-now usan la política de harness del bot (resolución en el despacho; precedencia pin > bot > defaults) | R16 | GLM | fusionada (#307; #188 cerrado) | #307 |
| R16-BF: paridad de latencia de arranque y apertura de páginas con orca-drogon en el paquete (`scripts/perf/measure-navigation.mjs`, `docs/reference/perf-navigation.md`) | R16 | Muse | fusionada (#306: todas las páginas 0.09–0.66× la referencia y < 300 ms; Tasks en frío pagaba el viaje a gh (1368 ms) → semilla en localStorage + revalidación, 58 ms; arranque ~0.8 s; cambio entre 20 tabs 71 ms; pendiente de decisión: cadencia ~1.3 s de la tarjeta de worktree) | #306 |
| R16-BG: estado de agente para Codex (working/needs_input/idle) vía CODEX_HOME gestionado por sesión, sin tocar ~/.codex; verificación con binario stub, sin inferencia pagada | R16 | luna | fusionada (#313: CODEX_HOME desechable por sesión con recursos/config espejados y hooks gestionados; Codex en bots, automations, settings y paleta) | #313 |
...
| CI R16-BL: #312 el job ubuntu de Foundation muere en el paso de tests del desktop con SIGTERM tras los mismos 49 archivos desde b1beaa6 (todos los PRs) | R16 | GLM | fusionada (#325: causa raíz = reciclado de workers de vitest por archivo; `isolate: false` + timeout de 15 min). #312 REABIERTO: main b341bf7 sigue muriendo en ubuntu tras 51 archivos; agrupar por entorno resultó peor (19 fallos, 20x más lento) y quedó documentado | #325 |
...
| QA fix R16-BM: #316 composer "Create worktree" del fork item por item (Project, Run on, Agent, Advanced) | R16 | Kimi k3 | fusionada (#327; #316 cerrado; restos en R16-BM2: filas Advanced — Branch name, Parent worktree, Note, Setup, Sparse — y Quick Session) | #327 |
| QA fix R16-BN: #315 Monaco añade un carácter al guardar tras seleccionar todo | R16 | Kimi | fusionada (#324; #315 cerrado: era el auto-cierre de Monaco al teclear, idéntico al fork; tests byte a byte) | #324 |
| R16-BO: #319 icono construido y verificado antes de instalar; install-preview tolera previews viejos | R16 | Muse | fusionada (#323; #319 cerrado) | #323 |
| R16-BP: seguridad de upgrade del data dir (inventario, migraciones con fixtures, backup previo, rechazo claro al degradar; probado desde f1f690f) | R16 | GLM | fusionada (#337: inventario de stores, dos huecos de versión futura cerrados, backup previo a migrar con manifiesto (N=3), diálogo de rechazo al degradar, fixtures v1/v2 y check opcional `upgrade-from-previous-build`; probado f1f690f → candidato) | #337 |
| R16-BQ: higiene para la entrega — gates `check:hygiene` (secretos/rutas/9445) y enlaces de docs en CI, cabeceras MIT en 34 archivos nuevos, screenshots frescos, `docs/SUBMISSION.md` | R16 | Muse | fusionada | #326 |
| R16-BM2: composer — filas Advanced (Branch name, Parent worktree, Note, Setup, Sparse) y Quick Session con capa de datos | R16 | Kimi k3 | lanzada | |
| R16-BR: Settings → Terminal, filas Interaction/Advanced (copy-on-select, right-click paste, focus-follows-mouse, OSC 52, scrollback, word separators) con consumidores xterm reales | R16 | luna | lanzada | |
| R16-BS: Notifications → Agent Task Complete y Terminal Bell con fuentes de evento reales | R16 | luna | lanzada | |
| Congelación de lanzamientos (Carlos, 2026-09-08 10:05): terminar los workers en curso, no lanzar nuevos (ni rondas de QA) hasta que la levante | R16 | Coordinador | vigente | |
| Cuota de Kimi agotada (403, ventana de 5 h) ~11:40: R16-BB (journeys sellados), R16-BH (menús/diálogos) y R16-BM2 (composer Advanced) quedaron a medias con sus ramas empujadas; no se relanzan por la congelación | R16 | Coordinador | bloqueadas | |
| QA funcional ronda 12: no verificables de r11 (#311 tarjeta < 1 s, #187 bot headless sin extensiones del usuario) + barrido adversarial (rutas con espacios/unicode, 5 MB/20k líneas, 300 commits, gh sin red, 50k líneas, quit en turno) + pase solo con teclado (124/124 atajos) | QA | luna | hecha: sin defectos nuevos | |
...
Un agente QA (Pi con Muse Spark 1.3 Contributor; las sesiones que lanza dentro de la
app usan solo el modelo local `dgx-spark/qwen3.8-flash-next-nvidia-nvfp4`, sin costo
por token) usa la app compilada desde `origin/main` como un usuario:
