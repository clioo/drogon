...
impl Engine {
    ...
    fn dispatch_inner(&self, request: &Request) -> Result<Value, RpcError> {
        match request.method.as_str() {
            ...
            "graph.observability_status" => self.graph_observability_status(&request.params),
            "graph.evidence_append" => self.graph_evidence_append(request),
            "graph.usage_append" => self.graph_usage_append(request),
