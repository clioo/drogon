...
// src/renderer/src/components/github/github-user-avatar.tsx (subset used by
// the task rows: avatar resolution, initials fallback and the retry-on-src-
// change behavior from #8784).
...
function initialsFor(login: string, name?: string | null): string {
  ...
  const alnum = (word: string): string[] =>
    [...word].filter((ch) => /[\p{L}\p{N}]/u.test(ch));
...
export function GitHubUserAvatar({
  ...
  return (
    <span
      title={title}
      className={cn(
        "flex shrink-0 items-center justify-center rounded-full bg-muted text-[9px] font-medium text-muted-foreground",
        className,
      )}
    >
