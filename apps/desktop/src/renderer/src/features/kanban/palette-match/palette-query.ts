/* MIT Copyright (c) 2026 Lovecast Inc. Ported verbatim from Orca
   src/renderer/src/lib/palette-match/palette-query.ts at pinned source c9790628
...
function parseRepoBranch(
  ...
  return {
    repo: text.slice(0, slashIndex),
    branch: text.slice(slashIndex + 1),
  };
}

export function createPaletteQueryToken(
  text: string,
  index: number,
): PaletteQueryToken {
  ...
  return {
    ...
    isSingleLatinCharacter:
      characters.length === 1 && LATIN_OR_DIGIT.test(text),
