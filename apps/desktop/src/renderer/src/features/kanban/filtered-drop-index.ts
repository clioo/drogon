/* MIT Copyright (c) 2026 Lovecast Inc. Ported verbatim from Orca
   src/renderer/src/components/sidebar/workspace-kanban-filtered-drop-index.ts at pinned source
   c9790628 (clioo/drogon-orca). Only this header, the module name and
   type-only imports were adapted. */
 ...
 */
export function serializeWorkspaceLaneFullIds(
  worktreeIds: readonly string[],
): string | null {
  if (
    worktreeIds.some((worktreeId) =>
      worktreeId.includes(WORKSPACE_LANE_FULL_IDS_DELIMITER),
    )
  ) {
    return null;
  }
...
export function parseWorkspaceLaneFullIds(
...
export function resolveFullLaneDropIndex(args: {
  ...
  if (filteredDropIndex >= renderedIds.length) {
    const lastIndex = indexInFullLane(
      fullLaneIds,
      renderedIds.at(-1)!,
      fullLaneIds.length - 1,
    );
    return Math.min(fullLaneIds.length, lastIndex + 1);
  }
  return indexInFullLane(
    fullLaneIds,
    renderedIds[filteredDropIndex]!,
    fullLaneIds.length,
  );
}
