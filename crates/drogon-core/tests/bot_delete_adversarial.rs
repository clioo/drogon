...
impl Fx {
    fn new() -> Self {
        ...
        Self {
            dir,
            engine,
            ws: ws["id"].as_str().unwrap().into(),
            host: ws["hostId"].as_str().unwrap().into(),
        }
    }
    ...
    fn responsibility(&self, request: &str, bot: &str) -> (String, String) {
        let v = ok(self.engine.dispatch(req(
            request,
        ...
        (
            v["responsibilityId"].as_str().unwrap().into(),
            v["automationId"].as_str().unwrap().into(),
        )
    }
    fn delete(&self, request: &str, scope: &str, host: &str, bot: &str) -> Response {
        self.engine.dispatch(req(
            request,
            "bot.delete",
            json!({"workspaceId": scope, "hostId": host, "botId": bot}),
        ))
    }
    ...
    fn all_automation_ids(&self) -> Vec<String> {
        ok(self
            .engine
            .dispatch(req("list-unique", "automation.list", json!({}))))["automations"]
            .as_array()
            .unwrap()
            .iter()
...
fn several_foreign_unowned_zero_replay_and_scopes() {
    ...
    assert_eq!(
        err(fx.delete("zero-delete-again", &fx.ws, &fx.host, &zero)),
        "not_found"
    );
    ...
    let mut receipt_ids: Vec<String> = params_receipt["automationIds"]
        .as_array()
        .unwrap()
        .iter()
        ...
        .collect();
    receipt_ids.sort();
    let mut expected = vec![owned_a.clone(), owned_b.clone()];
    expected.sort();
    ...
    ok(fx.engine.dispatch(req(
        "other-delete",
        "automation.delete",
        json!({"id": foreign_owned}),
    )));
    ...
    let other_ws = ok(fx.engine.dispatch(req(
        "ws-other",
        "workspace.register",
    ...
    assert_eq!(
        err(fx.delete("scope-ghost", "garbage-workspace", &fx.host, &ghost)),
        "unknown_workspace"
    );
    assert_eq!(
        err(fx.delete("scope-host", "", "wrong-host", &ghost)),
        "foreign_workspace_host"
    );
    assert_eq!(
        err(fx.delete("scope-bot-home-wrong-host", &fx.ws, "wrong-host", &ghost)),
        "foreign_workspace_host"
    );
}
...
fn responsibility_runs_survive_and_monitor_is_not_a_live_bot_after_delete() {
    ...
    let monitor = fx.engine.dispatch(req(
        "monitor",
        "bot.monitor_create",
        json!({
    ...
    assert_eq!(
        err(fx.engine.dispatch(req(
            "list-deleted-monitor",
            "bot.monitor_list",
            json!({
    ...
    let monitor_rows: i64 = db
        .query_row(
            "SELECT COUNT(*) FROM bot_monitors WHERE bot_id = ?1",
            [&bot],
            |r| r.get(0),
        )
        .unwrap();
    assert_eq!(
        monitor_rows, 1,
        "the current contract leaves the monitor as inert orphaned state"
    );
    let run_rows: i64 = db
        .query_row(
            "SELECT COUNT(*) FROM bot_responsibility_runs WHERE id = 'orphan-run-383'",
            [],
            |r| r.get(0),
        )
        .unwrap();
    assert_eq!(
        run_rows, 1,
        "responsibility-run evidence must survive Bot deletion"
    );
}
...
fn held_database_lock_rolls_back_the_entire_delete() {
    ...
    assert!(
        !failed.ok,
        "a held SQLite write lock must not report a successful delete: {failed:?}"
    );
    drop(lock);
    let listed = fx.all_automation_ids();
    assert!(
        listed.contains(&automation),
        "failed cascade must not delete the automation"
    );
    let snapshot = ok(fx.engine.dispatch(req(
        "locked-snapshot",
        "bot.snapshot",
        json!({
    ...
    assert!(
        snapshot["bots"]
            ...
            .unwrap()
            .iter()
            .any(|entry| entry["id"] == bot),
        "failed cascade must not delete the bot"
    );
}
...
fn two_deletes_and_delete_create_race_do_not_strand_an_owned_automation() {
    ...
    let t1 = thread::spawn(move || {
        let e = Engine::open(&db_path).unwrap();
        b1.wait();
        e.dispatch(req(
            "race-delete-a",
            "bot.delete",
            json!({"workspaceId":"", "hostId":host, "botId":bot_a}),
        ))
    });
    let db_path = fx.dir.path().to_path_buf();
    let host = fx.host.clone();
    ...
    let t2 = thread::spawn(move || {
        ...
        e.dispatch(req(
            "race-delete-b",
            "bot.delete",
            json!({"workspaceId":"", "hostId":host, "botId":bot_b}),
        ))
    });
    barrier.wait();
    ...
    assert_eq!(
        [r1.ok, r2.ok].iter().filter(|x| **x).count(),
        1,
        "one concurrent delete must win: {r1:?} {r2:?}"
    );
    ...
    let td = thread::spawn(move || {
        ...
        b3.wait();
        e.dispatch(req(
            "race-delete-create-delete",
            "bot.delete",
            json!({"workspaceId":ws,"hostId":host,"botId":bot_d}),
        ))
    });
    let db_path = fx.dir.path().to_path_buf();
    let host = fx.host.clone();
    ...
    let tc = thread::spawn(move || {
        let e = Engine::open(&db_path).unwrap();
        b4.wait();
        e.dispatch(req("race-delete-create-create", "bot.responsibility_create", json!({"workspaceId":ws,"hostId":host,"botId":bot_c,"name":"racing","schedule":"59 23 31 12 *","prompt":"fixture"})))
    });
    let rd = td.join().unwrap();
    let rc = tc.join().unwrap();
    let ids = fx.all_automation_ids();
    assert!(
        ids.is_empty(),
        "race must leave no ownerless automation: delete={rd:?} create={rc:?} ids={ids:?}"
    );
    let _ = initial;
}
