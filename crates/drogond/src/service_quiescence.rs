...
struct ConnectionEntry {
    id: usize,
    /// Clone of the handler's transport, used only to tear the socket down
    /// during drain; `shutdown(2)` applies to the socket itself, so the
    /// handler's own descriptor wakes immediately. Dropped when the
...
impl ConnectionRegistry {
    ...
    pub fn register(self: &Arc<Self>, transport: UnixStream) -> ActiveConnection {
    ...
    ///
    /// Every registered transport is shut down in both directions first, so
    /// handlers blocked in a read (an idle client) or write unwind promptly
    /// instead of waiting out the idle timeout. Each handler's completion is
    /// then awaited up to the deadline; handlers still running past it are
    /// detached, because a blocking wait would undo the bound. A detached
    /// handler can no longer serve — its socket is shut down and the
