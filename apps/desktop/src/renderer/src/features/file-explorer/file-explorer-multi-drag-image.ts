...
// src/renderer/src/components/right-sidebar/file-explorer-multi-drag-image.ts
// (verbatim DOM/styles; basename is inlined since this repo has no shared
// path lib).
...
export function createMultiSelectDragGhost(paths: string[], rowW: number): HTMLDivElement {
  ...
  for (const path of paths.slice(0, MAX_SHOWN)) {
    ...
    ghost.appendChild(makeRow(base));
  }
