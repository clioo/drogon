...
export const DESKTOP_TERMINAL_SCROLLBACK_ROW_PRESETS = [
...
export function normalizeDesktopTerminalScrollbackRows(value: unknown): number {
  if (!isFiniteNumber(value)) return DESKTOP_TERMINAL_SCROLLBACK_ROWS_DEFAULT;
...
export function normalizeDesktopTerminalSnapshotRows(
  value: unknown,
): number | undefined {
