/* MIT Copyright (c) 2026 Lovecast Inc. Ported verbatim from Orca
   src/renderer/src/lib/palette-match/match-quality.ts at pinned source c9790628
...
export function isFuzzyPaletteMatchQuality(
  quality: PaletteMatchQuality,
): boolean {
  return quality === "typo";
}

export function isExactPaletteMatchQuality(
  quality: PaletteMatchQuality,
): boolean {
  return quality === "field-exact" || quality === "word-exact";
}

export function isPrefixOrBoundaryPaletteMatchQuality(
  quality: PaletteMatchQuality,
): boolean {
  return (
...
export type PaletteResultQualityClass =
  (typeof PALETTE_RESULT_QUALITY_CLASSES)[number];

const CLASS_RANK = new Map<PaletteResultQualityClass, number>(
  PALETTE_RESULT_QUALITY_CLASSES.map((value, index) => [value, index]),
);

export function paletteResultQualityClassRank(
  value: PaletteResultQualityClass,
): number {
...
export function resolvePaletteResultQualityClass(args: {
  ...
  if (isContainerOnly) {
    return isExactPaletteMatchQuality(worstQuality)
      ? "exact-evidence"
      : "partial-evidence";
  }
