...
impl Fixture {
    ...
    /// recorded fixture children to observe it (or self-expire). Never
    /// signals a discovered PID.
    pub fn cleanup(&self) {
        ...
        while Instant::now() < deadline {
            ...
            if pids.is_empty() || pids.iter().all(|pid| !liveness_probe(*pid)) {
                match std::fs::remove_file(self.dir.join("env-dump.cap")) {
                    ...
                    Err(error) => panic!("remove synthetic credential fixture: {error}"),
                }
                std::fs::remove_dir_all(&self.dir).expect("remove owned synthetic fixture");
                return;
            }
