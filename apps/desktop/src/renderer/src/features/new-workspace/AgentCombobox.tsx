/* MIT Copyright (c) 2026 Lovecast Inc. Ported from Orca's
   src/renderer/src/components/agent/AgentCombobox.tsx (adapter: the catalog
   entries are this repo's daemon-listed Harnesses — only the harnesses Drogon
   ships, with the + menu's HarnessMenuIcon glyphs; the source's i18n keys are
   inlined as their English literals. "Blank Terminal" is the fork's no-agent
   choice: the workspace is created without starting a session). */
...
export type ComposerAgentOption = AgentPickerEntry & { id: HarnessId };

type DefaultAgentPreference = HarnessId | "blank" | null;

type AgentComboboxProps = {
  agents: ComposerAgentOption[];
  value: HarnessId | null;
  onValueChange: (agent: HarnessId | null) => void;
  onValueSelected?: (agent: HarnessId | null) => void;
...
export default function AgentCombobox({
  ...
  const selectedAgent = useMemo<ComposerAgentOption | null>(
            ...
            >
              {selectedAgent ? (
                <AgentIconLabel
                  icon={
                    <HarnessMenuIcon
                      harnessId={selectedAgent.id}
                      displayName={selectedAgent.label}
                      size={14}
                    />
                  }
                ...
                <AgentIconLabel
                  ...
                  label={emptyLabel ?? "Blank Terminal"}
                />
              )}
            ...
            <CommandList>
              ...
              {filteredAgents.map((agent) =>
                renderItem({
                  ...
                  icon: <HarnessMenuIcon harnessId={agent.id} displayName={agent.label} />,
                  label: agent.label,
                }),
              )}
            </CommandList>
